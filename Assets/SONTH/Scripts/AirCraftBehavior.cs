...
public class AirCraftBehavior : MonoBehaviour
{
    ...
    public void FireMachineGun()
    {
        ...
        Vector3 dir = LocalPlayer.Instance.GetTranformPlayer().position - _machineGun.transform.position;
        Quaternion rotation = Quaternion.LookRotation(dir);
        //_machineGun.transform.rotation = rotation;

        var bullet = ObjectPool.Instance.PopFromPool(_bullet, instantiateIfNone: true);
        bullet.transform.SetPositionAndRotation(_posSpawnBullet.position, rotation);
        ...
        bullet.transform.SetPositionAndRotation(_posSpawnBullet1.position, rotation);
    ...
    private void OnDrawGizmos()
    {
        ...
        for (int i = 0; i < paths.Length; i++)
        {
            for (int j = 0; j < number; j++)
            {
                ...
                Vector3 point1 = GetPoinInBezierCurve(t1, paths[i].pathBase.startPoint.position, paths[i].pathBase.controlPoint.position, paths[i].pathBase.endPoint.position);
                Vector3 point2 = GetPoinInBezierCurve(t2, paths[i].pathBase.startPoint.position, paths[i].pathBase.controlPoint.position, paths[i].pathBase.endPoint.position);
