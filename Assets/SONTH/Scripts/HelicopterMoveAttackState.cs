...
public class HelicopterMoveAttackState : BaseState<HelicopterState>
{
    ...
    [SerializeField] private float _machineGunDameBullet;
    ...
    public void FireMachineGun()
    {
        ...
        Vector3 dir = (LocalPlayer.Instance.GetTranformPlayer().position - _posSpawnBulletMachineGun.position).normalized;
        Quaternion rotation = Quaternion.LookRotation(dir);
    ...
    IEnumerator FireInfinityMachineGun()
    {
        ...
        while (_isFireMachineGunInfinity)
        {
            ...
            if (timeElapse >= _timeDelaySpawnBulletMachineGun)
            {
                timeElapse = 0f;
                FireMachineGun();
                EventManager.Invoke(EventName.OnTakeDamagePlayer, _machineGunDameBullet);
            }
            yield return null;
        }
