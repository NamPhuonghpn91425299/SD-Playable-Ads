...
public class PlayerView : MonoBehaviour
{
    ...
    [SerializeField] private Vector2 _initRotate;
    [SerializeField] private Vector2 _totalRotate;
    [SerializeField] private bool WeaponView = false;
    [SerializeField] private Transform WeaponTrans;
    [SerializeField] private Transform[] ListWeaponTrans;
    [SerializeField] private RectTransform CrossHair;
    [SerializeField] private Vector2 _crossHairMovementLimit = new Vector2(100f, 100f);
    [SerializeField] private Vector2 _weaponMovementLimit = new Vector2(30f, 30f);
    [SerializeField] private Vector2 screenPosValue;
    [SerializeField] private Transform CameraTrans;
    [SerializeField] private float CrossHairPos;
    [SerializeField] private float rollMultiplier = -0.5f; // Roll = Yaw * rollMultiplier
    ...
    public void Update()
    {
        if (Input.GetMouseButton(0))
        {
            ...
            if (!UIManager.Instance.isCanTouch)
            {
                ...
                {
                    float yaw = totalRotate.x;
                    float pitch = totalRotate.y;
                    float roll = yaw * rollMultiplier;

                    _mainRoot.localRotation = Quaternion.Slerp(_mainRoot.localRotation,
                        Quaternion.Euler(0, yaw, 0), slerpParam);

                    _head.localRotation = Quaternion.Slerp(_head.localRotation,
                        Quaternion.Euler(-pitch, 0, roll), slerpParam);
                }
            }
    ...
    private void OnChangeMachineGun(bool IsChange)
    {
        WeaponTrans = IsChange ? ListWeaponTrans[1] : ListWeaponTrans[0];
    }
