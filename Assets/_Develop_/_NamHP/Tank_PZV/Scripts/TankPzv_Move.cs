...
/// Date: 2024
/// </summary>
public class TankPzv_Move : StateBase
{
    [Header("=== VISUAL ===")]
    ...
    private void CreatePhase1PathSequence()
    {
        ...
        for (int i = currentPointIndex; i < assignedPath.points.Count; i++)
        {
            ...
            // Tạo ROTATE TWEEN - xoay về hướng target (chỉ xoay X,Y - cố định Z)
            // rotDuration = 70% của move time để xoay kịp trước khi đến nơi
            float rotDuration = Mathf.Min(moveDuration * 0.7f, rotationDuration);
            
            // Tính toán rotation với Z cố định
            Vector3 directionToTarget = (targetPos - TF.position).normalized;
            directionToTarget.y = 0; // Loại bỏ component Y để chỉ xoay trên mặt phẳng XZ
            Quaternion targetRotation = Quaternion.LookRotation(directionToTarget, Vector3.up);
            // Giữ nguyên rotation Z ban đầu
            Vector3 eulerAngles = targetRotation.eulerAngles;
            eulerAngles.z = TF.eulerAngles.z; // Cố định Z
            targetRotation = Quaternion.Euler(eulerAngles);
            
            Tween rotateTween = TF.DORotateQuaternion(targetRotation, rotDuration)
    ...
    private void CreateDOPathMovement()
    {
        ...
        .OnUpdate(() => {
            // Cố định Z rotation trong quá trình di chuyển DOPath
            Vector3 currentEuler = TF.eulerAngles;
            currentEuler.z = 0f; // Hoặc giá trị Z ban đầu nếu muốn giữ
            TF.eulerAngles = currentEuler;
        })
        .OnStart(() => {
    ...
    private IEnumerator RotateToNextWaypoint()
    {
        ...
        while (true)
        {
            ...
            // Cố định Z rotation - chỉ xoay trên mặt phẳng XZ
            Vector3 targetEuler = targetRot.eulerAngles;
            Vector3 currentEuler = TF.eulerAngles;
            targetEuler.z = currentEuler.z; // Giữ nguyên Z
