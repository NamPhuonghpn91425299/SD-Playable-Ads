...
public class BotDeathHandler : MonoBehaviour
{
    ...
    public void OnBotDeath(Vector3 botPosition)
    {
        ...
        float distaceToCamera = Vector3.Distance(botPosition, mainCamera.transform.position);
        Vector3 botHeadPosition;
        if (distaceToCamera > 25f)
            botHeadPosition = botPosition + Vector3.up * (headOffset+1f);
        else
            botHeadPosition = botPosition + Vector3.up * headOffset;
